...
void SeedModel::setSeeds(const QVector<Seed> &seeds) {
    ...
    seeds_ = seeds;  // 赋值新的种子列表
    endResetModel();   // 通知视图模型重置完成，刷新视图
}

Seed SeedModel::getSeed(int row) const {
    if (row < 0 || row >= seeds_.size())
        return Seed();  // 或者其他默认值
    return seeds_[row];
}
...
int SeedModel::rowCount(const QModelIndex &parent) const {
    Q_UNUSED(parent)
    return seeds_.size(); // 返回当前持有的种子数量
}
...
QVariant SeedModel::data(const QModelIndex &index, int role) const {
    ...
    return QVariant::fromValue(seeds_[index.row()]);
}
...
void SeedModel::appendSeed(const Seed &seed) {
    beginInsertRows(QModelIndex(), seeds_.size(),
                    seeds_.size()); // 通知视图即将插入
    seeds_.push_back(seed);         // 添加数据
